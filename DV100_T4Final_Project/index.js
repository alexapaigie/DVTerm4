...
function clickme() {
    ...
    if (user == false) {
        window.location = "whoswatching.html"
...
function signup() {
    ...
    var npass = document.getElementById("password1").value;
    var npasscon = document.getElementById("password2").value;


    for (let i = 0; i < users.length; i++) {
        if (nusername == users[i].username) {
            exists = false;
        } else {
            if (npass == npasscon) {
                passgood = true;
    ...
    if (exists) {
        if (passgood = true) {
            ...
            users.push(newuser);
            console.log(users);
            var storeusers = JSON.stringify(users)
            localStorage.setItem("usernames", storeusers)
            savename = JSON.stringify(nusername)
...
function loadhome() {
    $(document).ready(function () {
        ...
        var welcomename = JSON.parse(getname);

        $("#welcomeuser").append(welcomename);
        $("#ToastName").append(welcomename + "!!");
    })
}
    ...
    $(document).ready(function () {
        ...
        $.ajax({
            ...
            for (let x = 21; x < 26; x++) {
                ...
                if (info2.results[x].genre_ids[1] == 28) {
...
//POPULATE INDIVIDUAL MOVIE PAGE
function populate() {
    $(document).ready(function () {
        var movies = "";
        var actors = "";
        ...
        var rec2 = null;
        var rec3 = null;
        $.getJSON("https://api.themoviedb.org/3/movie/" + idnr + "/recommendations?api_key=330d88838c8de52c3cf7278db6c50a18&language=en-US&page=1", function (data) {
            $("#rec1title").append(data.results[0].title)
            $("#rec1syn").append(data.results[0].overview)
            $("#rec1poster").attr("src", "https://image.tmdb.org/t/p/w500/" + data.results[0].poster_path)
            rec1 = JSON.stringify(data.results[0].id)
            localStorage.setItem("rec1id", rec1)

            $("#rec2title").append(data.results[1].title)
            $("#rec2syn").append(data.results[1].overview)
            $("#rec2poster").attr("src", "https://image.tmdb.org/t/p/w500/" + data.results[1].poster_path)
            rec2 = JSON.stringify(data.results[1].id)
            localStorage.setItem("rec2id", rec2)

            $("#rec3title").append(data.results[2].title)
            $("#rec3syn").append(data.results[2].overview)
            $("#rec3poster").attr("src", "https://image.tmdb.org/t/p/w500/" + data.results[2].poster_path)
            rec3 = JSON.stringify(data.results[2].id)
            localStorage.setItem("rec3id", rec3)
        })
    })
}


//LISTS ON LIBRARY PAGE
var lists = "top_rated"
function upcoming() {
    lists = "upcoming"
    loadlib();
...
function loadlib() {
    $(document).ready(function () {
        ...
        $.ajax({
            type: "GET",
            url: "https://api.themoviedb.org/3/movie/" + lists + "?api_key=330d88838c8de52c3cf7278db6c50a18&language=en-US&page=1",
            success: function (data) {
                info = data;
            }
        }).done(function () {
            ...
            for (let i = 0; i < 20; i++) {
                ...
                $("#movie" + i + "title").append("<span style='background-color: rgb(48, 41, 41, 0.573);'>" + info.results[i].title + "</span>");
                $("#movie" + i + "syn").append("<span style='background-color: rgb(48, 41, 41, 0.573);'>" + info.results[i].overview + "</span>");
        ...
        $.ajax({
            type: "GET",
            url: "https://api.themoviedb.org/3/movie/" + lists + "?api_key=330d88838c8de52c3cf7278db6c50a18&language=en-US&page=2",
            success: function (data) {
                info2 = data;
            }
        }).done(function () {
            ...
            for (let x = 20; x < 28; x++) {
                ...
                $("#movie" + x + "title").append("<span style='background-color: rgb(48, 41, 41, 0.573);'>" + info2.results[count].title + "</span>");
                $("#movie" + x + "syn").append("<span style='background-color: rgb(48, 41, 41, 0.573);'>" + info2.results[count].overview + "</span>");
...
//HOME PAGE - move to individual page
function move1() {
...
function rec1btn() {
    var get = localStorage.getItem("rec1id")
    nextid = get;
    localStorage.setItem("nextid", nextid)
}

function rec2btn() {
    var get = localStorage.getItem("rec2id")
    nextid = get;
    localStorage.setItem("nextid", nextid)
}

function rec3btn() {
    var get = localStorage.getItem("rec3id")
    nextid = get;
    localStorage.setItem("nextid", nextid)
}
...
//LIBRARY - move to individual movie page
function movie1() {
...
$(document).ready(function () {
    ...
    $("#movie26syn").hide();
    $("#movie26").mouseenter(function () {
        $("#movie26syn").slideDown();
    });
    $("#movie26").mouseleave(function () {
        $("#movie26syn").slideUp();
    });

    $("#movie27syn").hide();
    $("#movie27").mouseenter(function () {
        $("#movie27syn").slideDown();
    });
    $("#movie27").mouseleave(function () {
        $("#movie27syn").slideUp();
    });
    ...
    $("#sim1syn").hide();
    $("#sim1").mouseenter(function () {
        $("#sim1syn").slideDown();
    });
    $("#sim1").mouseleave(function () {
        $("#sim1syn").slideUp();
    });

    $("#sim2syn").hide();
    $("#sim2").mouseenter(function () {
        $("#sim2syn").slideDown();
    });
    $("#sim2").mouseleave(function () {
        $("#sim2syn").slideUp();
    });

    $("#sim3syn").hide();
    $("#sim3").mouseenter(function () {
        $("#sim3syn").slideDown();
    });
    $("#sim3").mouseleave(function () {
        $("#sim3syn").slideUp();
    });
